 ...
 * 3. 1行目に HEADER_ROW の列名をその順序で貼り付ける（既存列がある場合は Code.gs 側を合わせる）
...
/** 1行目のヘッダー（この順でデータを書き込みます）— LPは氏名・電話のみ */
var HEADER_ROW = [
  'タイムスタンプ',
  'フォーム種別',
  '店舗名',
...
function validateParams_(p) {
  ...
  if (!p.event_slot) throw new Error('参加希望レッスンは必須です。');
  if (!p.visit_date) throw new Error('参加希望日は必須です。');
